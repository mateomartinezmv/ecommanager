  ...
  try {
    ...
    // ── /ventas hoy ──────────────────────────────────────────
      ...
      if (!ventas || ventas.length === 0) {
        return sendTelegram(chatId, `📊 <b>Ventas hoy (${hoy})</b>\n\nSin ventas registradas por ahora.`);
      }
    ...
    // ── /ventas mes ──────────────────────────────────────────
      ...
      if (!ventas || ventas.length === 0) {
        return sendTelegram(chatId, `📅 <b>Ventas este mes</b>\n\nSin ventas registradas este mes.`);
      }
    ...
    // ── /envios ──────────────────────────────────────────────
      ...
      if (!envios || envios.length === 0) {
        return sendTelegram(chatId, '🚚 <b>Envíos pendientes</b>\n\nNo hay envíos pendientes. ✅');
      }
    ...
    // ── /ganancia ────────────────────────────────────────────
      ...
      const [ventasRes, productosRes] = await Promise.all([
        ...
        supabase.from('productos').select('sku, costo, nombre'),
      ]);
      ...
      if (ventas.length === 0) {
        return sendTelegram(chatId, '💰 <b>Ganancia este mes</b>\n\nSin ventas registradas este mes.');
      }

      const costoMap = {};
      productos.forEach(p => { costoMap[p.sku] = { costo: p.costo, nombre: p.nombre }; });

      let ingresos = 0, comisiones = 0, costos = 0;
      ventas.forEach(v => {
        ingresos += v.total;
        comisiones += v.comision || 0;
        const prod = costoMap[v.sku];
        if (prod) costos += prod.costo * v.cantidad;
      });
    ...
    // ── /recomendaciones ─────────────────────────────────────
      ...
      const productosSinVenta = productos.filter(p =>
        !ventas.find(v => v.sku === p.sku)
      ).map(p => p.nombre).slice(0, 5);

      const resumen = `
Soy dueño de una tienda de accesorios para motos en Uruguay llamada Martinez Motos. Vendemos por Mercado Libre y mostrador físico.
...
Dame 4 o 5 recomendaciones concretas, cortas y accionables para mejorar las ventas, el stock o la operación. Sé directo, sin vueltas. Formato: cada recomendación con un emoji relevante al inicio y máximo 2 líneas.
    ...
    await sendTelegram(chatId, '❌ Ocurrió un error procesando tu consulta. Revisá los logs de Vercel.');
  }
};
